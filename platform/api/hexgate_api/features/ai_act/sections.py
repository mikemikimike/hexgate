...
# Completeness, per the spec: purpose, operator role, risk tier and an
# oversight owner, plus an Annex III point when the asserted tier is
# high_risk. PR 1's endpoint applies the same rule on write; the report
# applies it on read because it has to label every row, including rows written
# before any rule existed and agents with no entry at all.
HIGH_RISK_TIER = "high_risk"
_REQUIRED_FIELDS = (
...
_ANNEX_III_FIELD = ("annex_iii_point", "Annex III reference")
...
def missing_inventory_fields(classification: Any | None) -> list[str]:
    """The human-readable names of the fields a classification entry lacks.

    An agent with no entry is missing all of them — that is what "incomplete"
    means for an agent nobody has classified yet.
    """
    if classification is None:
        return [label for _, label in _REQUIRED_FIELDS] + [_ANNEX_III_FIELD[1]]
    missing = [
        label
        for field, label in _REQUIRED_FIELDS
        if not getattr(classification, field, None)
    ]
    field, label = _ANNEX_III_FIELD
    if getattr(classification, "risk_tier", None) == HIGH_RISK_TIER and not getattr(
